...
def db_open_saving_account(db, cusID, accountID, money, settime, interestrate, saveType):
    ...
    try:
        ...
        db.commit()
        # 将创建时间作为初始的visit
        sql = "INSERT INTO cusforacc VALUES('%s', '建设银行', '%s', '%s', 'saving')" %(accountID, cusID, settime)
        cursor.execute(sql)
        db.commit()
...
def test_db_open_saving_account(db):
    cursor = db.cursor()
    try:
        sql = "INSERT INTO accounts VALUES('111', 1000, '2022-1-1', 'saving')" 
        cursor.execute(sql)
        sql = "INSERT INTO saveacc VALUES('111', 0.05, '1')" 
        cursor.execute(sql)
        # 将创建时间作为初始的visit
        sql = "INSERT INTO cusforacc VALUES('111', '建设银行', '123', '2022-1-1', 'saving')" 
        cursor.execute(sql)
        db.commit()
    except:
        db.rollback()
...
def db_open_checking_account(db, cusID, accountID, money, settime, overdraft):
    ...
    try:
        ...
        cursor.execute(sql)
        sql = "INSERT INTO cusforacc VALUES('%s', '建设银行', '%s', '%s', 'checking')" %(accountID, cusID, settime)
        cursor.execute(sql)
        db.commit()
    except:
        db.rollback()
...
def db_create_loan(db, loanID, cusID, money):
    cursor = db.cursor()
    try:
        sql = "INSERT INTO loan VALUES('%s', %s, '建设银行', '0')" %(loanID, money)
        cursor.execute(sql)
...
if __name__ == "__main__":
    db = db_login("root", "pb8893253", "127.0.0.1", "test")
